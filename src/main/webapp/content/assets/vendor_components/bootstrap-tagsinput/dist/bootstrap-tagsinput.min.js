    ...
    add: function (b, c, d) {
      ...
      if (!(f.options.maxTags && f.itemsArray.length >= f.options.maxTags) && (b === !1 || b)) {
        ...
        if (!b.toString().match(/^\s*$/)) {
          if (
            (f.isSelect && !f.multiple && f.itemsArray.length > 0 && f.remove(f.itemsArray[0]),
          ...
          if (!n || f.options.allowDuplicates) {
            if (!(f.items().toString().length + b.length + 1 > f.options.maxInputLength)) {
              ...
              if ((f.$element.trigger(o), !o.cancel)) {
                f.itemsArray.push(b);
                var p = a(
                  '<span class="tag ' + e(l) + (null !== m ? '" title="' + m : '') + '">' + e(k) + '<span data-role="remove"></span></span>'
                );
                if (
                  (p.data('item', b),
                  f.findInputWrapper().before(p),
                  p.after(' '),
                  f.isSelect && !a('option[value="' + encodeURIComponent(j) + '"]', f.$element)[0])
                ) {
                  var q = a('<option selected>' + e(k) + '</option>');
                  q.data('item', b), q.attr('value', j), f.$element.append(q);
                }
                c || f.pushVal(),
                  (f.options.maxTags === f.itemsArray.length || f.items().toString().length === f.options.maxInputLength) &&

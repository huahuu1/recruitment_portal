 ...
 *
 */ (function (e) {
  var t = { set: { colors: 1, values: 1, backgroundColor: 1, scaleColors: 1, normalizeFunction: 1, focus: 1 }, get: { selectedRegions: 1, selectedMarkers: 1, mapObject: 1, regionName: 1 } };
...
(jvm.$ = jQuery),
  ...
  (jvm.VMLElement.prototype.getBBox = function () {
    var e = jvm.$(this.node);
    return { x: e.position().left / this.canvas.scale, y: e.position().top / this.canvas.scale, width: e.width() / this.canvas.scale, height: e.height() / this.canvas.scale };
  }),
  ...
  (jvm.DataSeries.prototype = {
    ...
    setValues: function (e) {
      var t = Number.MIN_VALUE,
        ...
        i,
        s = {};
      if (this.scale instanceof jvm.OrdinalScale || this.scale instanceof jvm.SimpleScale) for (i in e) e[i] ? (s[i] = this.scale.getValue(e[i])) : (s[i] = this.elements[i].element.style.initial[this.params.attribute]);
      else {
  ...
  (jvm.Proj = {
    ...
    mill_inv: function (e, t, n) {
      return { lat: (2.5 * Math.atan(Math.exp((0.8 * t) / this.radius)) - (5 * Math.PI) / 8) * this.degRad, lng: (n * this.radDeg + e / this.radius) * this.degRad };
    },
    merc: function (e, t, n) {
    ...
    lcc_inv: function (e, t, n) {
      var r = e / this.radius,
        ...
        p = Math.atan(r / (c - i));
      return { lat: (2 * Math.atan(Math.pow(l / h, 1 / f)) - Math.PI / 2) * this.degRad, lng: (o + p / f) * this.degRad };
    },
  }),
  ...
  (jvm.WorldMap.prototype = {
    ...
    createLabel: function () {
      ...
      (this.label = jvm.$('<div/>').addClass('jvectormap-label').appendTo(jvm.$('body'))),
        this.container.mousemove(function (t) {
          ...
          n < 5 && (n = t.pageX + 15), r < 5 && (r = t.pageY + 15), e.label.is(':visible') && e.label.css({ left: n, top: r });
        });
    },
    ...
    setFocus: function (e, t, n) {
      ...
      if (jvm.$.isArray(e) || this.regions[e]) {
        ...
        for (u = 0; u < o.length; u++) this.regions[o[u]] && ((i = this.regions[o[u]].element.getBBox()), i && (typeof r == 'undefined' ? (r = i) : ((s = { x: Math.min(r.x, i.x), y: Math.min(r.y, i.y), width: Math.max(r.x + r.width, i.x + i.width) - Math.min(r.x, i.x), height: Math.max(r.y + r.height, i.y + i.height) - Math.min(r.y, i.y) }), (r = s))));
    ...
    createRegions: function () {
      ...
      for (e in this.mapData.paths)
        (t = this.canvas.addPath({ d: this.mapData.paths[e].path, 'data-code': e }, jvm.$.extend(!0, {}, this.params.regionStyle))),
    ...
    createMarkers: function (e) {
      ...
      for (t in e)
        (i = e[t] instanceof Array ? { latLng: e[t] } : e[t]),
          ...
          r !== !1 &&
            ((n = this.canvas.addCircle({ 'data-index': t, cx: r.x, cy: r.y }, jvm.$.extend(!0, {}, this.params.markerStyle, { initial: i.style || {} }), this.markersGroup)),
    ...
    repositionMarkers: function () {
      var e, t;
      for (e in this.markers) (t = this.getMarkerPosition(this.markers[e].config)), t !== !1 && this.markers[e].element.setStyle({ cx: t.x, cy: t.y });
    },
    ...
    latLngToPoint: function (e, t) {
      ...
      return t < -180 + i && (t += 360), (n = jvm.Proj[r.type](e, t, i)), (u = this.getInsetForPoint(n.x, n.y)), u ? ((a = u.bbox), (n.x = ((n.x - a[0].x) / (a[1].x - a[0].x)) * u.width * this.scale), (n.y = ((n.y - a[0].y) / (a[1].y - a[0].y)) * u.height * this.scale), { x: n.x + this.transX * this.scale + u.left * this.scale, y: n.y + this.transY * this.scale + u.top * this.scale }) : !1;
    },
  ...
  (jvm.WorldMap.maps = {}),
  (jvm.WorldMap.defaultParams = { map: 'world_mill_en', backgroundColor: '#505050', zoomButtons: !0, zoomOnScroll: !0, zoomMax: 8, zoomMin: 1, zoomStep: 1.6, regionsSelectable: !1, markersSelectable: !1, bindTouchEvents: !0, regionStyle: { initial: { fill: 'white', 'fill-opacity': 1, stroke: 'none', 'stroke-width': 0, 'stroke-opacity': 1 }, hover: { 'fill-opacity': 0.8 }, selected: { fill: 'yellow' }, selectedHover: {} }, markerStyle: { initial: { fill: 'grey', stroke: '#505050', 'fill-opacity': 1, 'stroke-width': 1, 'stroke-opacity': 1, r: 5 }, hover: { stroke: 'black', 'stroke-width': 2 }, selected: { fill: 'blue' }, selectedHover: {} } }),

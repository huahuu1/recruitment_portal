    ...
    var r = e('../lib/oop'),
      ...
      o = function () {
        var e = 'else|break|case|return|goto|if|const|select|continue|struct|default|switch|for|range|func|import|package|chan|defer|fallthrough|go|interface|map|range|select|type|var',
          ...
          s = this.createKeywordMapper({ keyword: e, 'constant.language': r, 'support.function': n, 'support.type': t }, ''),
        ...
        (this.$rules = {
          start: [
            ...
            { token: 'constant.numeric', regex: "'(?:[^\\'\ud800-\udbff]|[\ud800-\udbff][\udc00-\udfff]|" + o.replace('"', '') + ")'" },
            { token: 'constant.numeric', regex: '0[xX][0-9a-fA-F]+\\b' },
            { token: 'constant.numeric', regex: '[+-]?\\d+(?:(?:\\.\\d*)?(?:[eE][+-]?\\d+)?)?\\b' },
            { token: ['keyword', 'text', 'entity.name.function'], regex: '(func)(\\s+)([a-zA-Z_$][a-zA-Z0-9_$]*)\\b' },
            {
              token: function (e) {
                return e[e.length - 1] == '('
                  ? [

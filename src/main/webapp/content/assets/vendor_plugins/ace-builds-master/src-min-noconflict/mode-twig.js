    ...
    var r = e('../lib/oop'),
      ...
      u = function () {
        ...
        var t = 'abs|batch|capitalize|convert_encoding|date|date_modify|default|e|escape|first|format|join|json_encode|keys|last|length|lower|merge|nl2br|number_format|raw|replace|reverse|slice|sort|split|striptags|title|trim|upper|url_encode',
          ...
          u = this.createKeywordMapper({ 'keyword.control.twig': e, 'support.function.twig': [t, n, r].join('|'), 'keyword.operator.twig': o, 'constant.language.twig': i }, 'identifier');
        ...
        (this.$rules['twig-comment'] = [{ token: 'comment.block.twig', regex: '.*-?#\\}', next: 'pop' }]),
          ...
          (this.$rules['twig-qqstring'] = [{ token: 'constant.language.escape', regex: /\\[\\"$#ntr]|#{[^"}]*}/ }, { token: 'string', regex: '"', next: 'twig-start' }, { defaultToken: 'string' }]),
          (this.$rules['twig-qstring'] = [{ token: 'constant.language.escape', regex: /\\[\\'ntr]}/ }, { token: 'string', regex: "'", next: 'twig-start' }, { defaultToken: 'string' }]),
          this.normalizeRules();
      };

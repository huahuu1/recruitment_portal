ace.define('ace/mode/latex_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text_highlight_rules'], function (e, t, n) {
  ...
  var r = e('../lib/oop'),
    ...
    s = function () {
      this.$rules = {
        start: [
          { token: 'comment', regex: '%.*$' },
          { token: ['keyword', 'lparen', 'variable.parameter', 'rparen', 'lparen', 'storage.type', 'rparen'], regex: '(\\\\(?:documentclass|usepackage|input))(?:(\\[)([^\\]]*)(\\]))?({)([^}]*)(})' },
          { token: ['keyword', 'lparen', 'variable.parameter', 'rparen'], regex: '(\\\\(?:label|v?ref|cite(?:[^{]*)))(?:({)([^}]*)(}))?' },
          { token: ['storage.type', 'lparen', 'variable.parameter', 'rparen'], regex: '(\\\\(?:begin|end))({)(\\w*)(})' },
        ...
        equation: [{ token: 'comment', regex: '%.*$' }, { token: 'string', regex: '\\${1,2}', next: 'start' }, { token: 'constant.character.escape', regex: '\\\\(?:[^a-zA-Z]|[a-zA-Z]+)' }, { token: 'error', regex: '^\\s*$', next: 'start' }, { defaultToken: 'string' }],
      };
    };
        ...
        (this.foldingStartMarker = /^\s*\\(begin)|(section|subsection|paragraph)\b|{\s*$/),
          ...
          (this.latexBlock = function (e, t, n) {
            var r = { '\\begin': 1, '\\end': -1 },
              i = new o(e, t, n),
              u = i.getCurrentToken();
            if (!u || (u.type != 'storage.type' && u.type != 'constant.character.escape')) return;
            var a = u.value,
              f = r[a],
              l = function () {
                var e = i.stepForward(),
            ...
            while ((u = i.step())) {
              if (!u || (u.type != 'storage.type' && u.type != 'constant.character.escape')) continue;
              var d = r[u.value];
              if (!d) continue;
              var v = l();
              if (d === f) c.unshift(v);
          ...
          (this.latexSection = function (e, t, n) {
            var r = ['\\subsection', '\\section', '\\begin', '\\end', '\\paragraph'],
              i = new o(e, t, n),
              u = i.getCurrentToken();
            if (!u || u.type != 'storage.type') return;
            var a = r.indexOf(u.value),
              f = 0,
              l = t;
            while ((u = i.stepForward())) {
              if (u.type !== 'storage.type') continue;
  ...
  ace.define('ace/mode/latex', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/latex_highlight_rules', 'ace/mode/behaviour/cstyle', 'ace/mode/folding/latex'], function (e, t, n) {
    'use strict';
    var r = e('../lib/oop'),
      i = e('./text').Mode,

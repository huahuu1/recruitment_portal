ace.define('ace/mode/nsis_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text_highlight_rules'], function (e, t, n) {
  ...
  var r = e('../lib/oop'),
    ...
    s = function () {
      (this.$rules = {
        start: [
          { token: 'keyword.compiler.nsis', regex: /^\s*!(?:include|addincludedir|addplugindir|appendfile|cd|delfile|echo|error|execute|packhdr|finalize|getdllversion|system|tempfile|warning|verbose|define|undef|insertmacro|macro|macroend|makensis|searchparse|searchreplace)\b/, caseInsensitive: !0 },
          ...
          { token: 'constant.language.boolean.true.nsis', regex: /\b(?:true|on)\b/ },
          { token: 'constant.language.boolean.false.nsis', regex: /\b(?:false|off)\b/ },
          { token: 'constant.language.option.nsis', regex: /(?:\b|^\s*)(?:(?:un\.)?components|(?:un\.)?custom|(?:un\.)?directory|(?:un\.)?instfiles|(?:un\.)?license|uninstConfirm|admin|all|auto|both|bottom|bzip2|current|force|hide|highest|ifdiff|ifnewer|lastused|leave|left|listonly|lzma|nevershow|none|normal|notset|right|show|silent|silentlog|textonly|top|try|user|Win10|Win7|Win8|WinVista|zlib)\b/, caseInsensitive: !0 },
          ...
          { token: 'constant.numeric.nsis', regex: /\b(?:0(?:x|X)[0-9a-fA-F]+|[0-9]+(?:\.[0-9]+)?)\b/ },
          { token: 'entity.name.function.nsis', regex: /\$\([\w\.:-]+\)/ },
          { token: 'storage.type.function.nsis', regex: /\$[\w]+/ },
          { token: 'punctuation.definition.string.begin.nsis', regex: /`/, push: [{ token: 'punctuation.definition.string.end.nsis', regex: /`/, next: 'pop' }, { token: 'constant.character.escape.nsis', regex: /\$\\./ }, { defaultToken: 'string.quoted.back.nsis' }] },
          { token: 'punctuation.definition.string.begin.nsis', regex: /"/, push: [{ token: 'punctuation.definition.string.end.nsis', regex: /"/, next: 'pop' }, { token: 'constant.character.escape.nsis', regex: /\$\\./ }, { defaultToken: 'string.quoted.double.nsis' }] },
          { token: 'punctuation.definition.string.begin.nsis', regex: /'/, push: [{ token: 'punctuation.definition.string.end.nsis', regex: /'/, next: 'pop' }, { token: 'constant.character.escape.nsis', regex: /\$\\./ }, { defaultToken: 'string.quoted.single.nsis' }] },
          { token: ['punctuation.definition.comment.nsis', 'comment.line.nsis'], regex: /(;|#)(.*$)/ },
          { token: 'punctuation.definition.comment.nsis', regex: /\/\*/, push: [{ token: 'punctuation.definition.comment.nsis', regex: /\*\//, next: 'pop' }, { defaultToken: 'comment.block.nsis' }] },
          { token: 'text', regex: /(?:!include|!insertmacro)\b/ },
        ],
      }),
        this.normalizeRules();
    };
  (s.metaData = { comment: '\n	todo: - highlight functions\n	', fileTypes: ['nsi', 'nsh'], name: 'NSIS', scopeName: 'source.nsis' }), r.inherits(s, i), (t.NSISHighlightRules = s);
}),

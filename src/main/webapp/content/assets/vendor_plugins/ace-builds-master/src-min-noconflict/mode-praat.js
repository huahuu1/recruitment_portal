ace.define('ace/mode/praat_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text_highlight_rules'], function (e, t, n) {
  'use strict';
  var r = e('../lib/oop'),
    ...
    s = function () {
      ...
      this.$rules = {
        start: [
          { token: 'string.interpolated', regex: /'((?:[a-z][a-zA-Z0-9_]*)(?:\$|#|:[0-9]+)?)'/ },
          { token: ['text', 'text', 'keyword.operator', 'text', 'keyword'], regex: /(^\s*)(?:([a-z][a-zA-Z0-9_]*\$?\s+)(=)(\s+))?(stopwatch)/ },
          { token: ['text', 'keyword', 'text', 'string'], regex: /(^\s*)(print(?:line|tab)?|echo|exit|pause|send(?:praat|socket)|include|execute|system(?:_nocheck)?)(\s+)(.*)/ },
          { token: ['text', 'keyword'], regex: '(^\\s*)(' + n + ')$' },
          { token: ['text', 'keyword.operator', 'text'], regex: /(\s+)((?:\+|-|\/|\*|<|>)=?|==?|!=|%|\^|\||and|or|not)(\s+)/ },
          { token: ['text', 'text', 'keyword.operator', 'text', 'keyword', 'text', 'keyword'], regex: /(^\s*)(?:([a-z][a-zA-Z0-9_]*\$?\s+)(=)(\s+))?(?:((?:no)?warn|(?:unix_)?nocheck|noprogress)(\s+))?((?:[A-Z][^.:"]+)(?:$|(?:\.{3}|:)))/ },
          { token: ['text', 'keyword', 'text', 'keyword'], regex: /(^\s*)(?:(demo)?(\s+))((?:[A-Z][^.:"]+)(?:$|(?:\.{3}|:)))/ },
          { token: ['text', 'keyword', 'text', 'keyword'], regex: /^(\s*)(?:(demo)(\s+))?(10|12|14|16|24)$/ },
          { token: ['text', 'support.function', 'text'], regex: /(\s*)(do\$?)(\s*:\s*|\s*\(\s*)/ },
          ...
          { token: ['text', 'keyword', 'text', 'entity.name.section'], regex: /(^\s*)(\bform\b)(\s+)(.*)/, next: 'form' },
          { token: 'constant.numeric', regex: /\b[+-]?\d+(?:(?:\.\d*)?(?:[eE][+-]?\d+)?)?\b/ },
          { token: ['keyword', 'text', 'entity.name.function'], regex: /(procedure)(\s+)(\S+)/ },
          { token: ['entity.name.function', 'text'], regex: /(@\S+)(:|\s*\()/ },
          { token: ['text', 'keyword', 'text', 'entity.name.function'], regex: /(^\s*)(call)(\s+)(\S+)/ },
          { token: 'comment', regex: /(^\s*#|;).*$/ },
          { token: 'text', regex: /\s+/ },
        ],
        form: [
          { token: ['keyword', 'text', 'constant.numeric'], regex: /((?:optionmenu|choice)\s+)(\S+:\s+)([0-9]+)/ },
          { token: ['keyword', 'constant.numeric'], regex: /((?:option|button)\s+)([+-]?\d+(?:(?:\.\d*)?(?:[eE][+-]?\d+)?)?\b)/ },
          { token: ['keyword', 'string'], regex: /((?:option|button)\s+)(.*)/ },
          { token: ['keyword', 'text', 'string'], regex: /((?:sentence|text)\s+)(\S+\s*)(.*)/ },
          { token: ['keyword', 'text', 'string', 'invalid.illegal'], regex: /(word\s+)(\S+\s*)(\S+)?(\s.*)?/ },
          { token: ['keyword', 'text', 'constant.language'], regex: /(boolean\s+)(\S+\s*)(0|1|"?(?:yes|no)"?)/ },
          { token: ['keyword', 'text', 'constant.numeric'], regex: /((?:real|natural|positive|integer)\s+)(\S+\s*)([+-]?\d+(?:(?:\.\d*)?(?:[eE][+-]?\d+)?)?\b)/ },
          { token: ['keyword', 'string'], regex: /(comment\s+)(.*)/ },
          { token: 'keyword', regex: 'endform', next: 'start' },
        ],
        for: [
          { token: ['keyword', 'text', 'constant.numeric', 'text'], regex: /(from|to)(\s+)([+-]?\d+(?:(?:\.\d*)?(?:[eE][+-]?\d+)?)?)(\s*)/ },
  ...
  ace.define('ace/mode/praat', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/praat_highlight_rules', 'ace/mode/matching_brace_outdent', 'ace/mode/folding/cstyle'], function (e, t, n) {
    'use strict';
    var r = e('../lib/oop'),
      i = e('./text').Mode,

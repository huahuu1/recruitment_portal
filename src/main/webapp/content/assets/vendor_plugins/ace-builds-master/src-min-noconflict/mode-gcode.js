  ...
  var r = e('../lib/oop'),
    ...
    s = function () {
      ...
      this.$rules = {
        start: [
          { token: 'comment', regex: '\\(.*\\)' },
          { token: 'comment', regex: '([N])([0-9]+)' },
          { token: 'string', regex: '([G])([0-9]+\\.?[0-9]?)' },
          { token: 'string', regex: '([M])([0-9]+\\.?[0-9]?)' },
          { token: 'constant.numeric', regex: '([-+]?([0-9]*\\.?[0-9]+\\.?))|(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?|\\.\\d+)([eE][-+]?\\d+)?)' },
          { token: r, regex: '[A-Z]' },
          { token: 'keyword.operator', regex: 'EQ|LT|GT|NE|GE|LE|OR|XOR' },

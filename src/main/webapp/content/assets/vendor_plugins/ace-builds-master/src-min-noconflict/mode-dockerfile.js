    ...
    u = function () {
      ...
      (this.$rules = {
        start: [
          ...
          { token: 'string.start', regex: '"', push: [{ token: 'constant.language.escape', regex: /\\(?:[$`"\\]|$)/ }, { include: 'variables' }, { token: 'keyword.operator', regex: /`/ }, { token: 'string.end', regex: '"', next: 'pop' }, { defaultToken: 'string' }] },
          { token: 'string', regex: "\\$'", push: [{ token: 'constant.language.escape', regex: /\\(?:[abeEfnrtv\\'"]|x[a-fA-F\d]{1,2}|u[a-fA-F\d]{4}([a-fA-F\d]{4})?|c.|\d{1,3})/ }, { token: 'string', regex: "'", next: 'pop' }, { defaultToken: 'string' }] },
          ...
          { token: ['keyword', 'text', 'text', 'text', 'variable'], regex: /(declare|local|readonly)(\s+)(?:(-[fixar]+)(\s+))?([a-zA-Z_][a-zA-Z0-9_]*\b)/ },
        ...
        variables: [
          ...
          { token: ['variable', 'paren.lparen', 'keyword.operator', 'variable', 'keyword.operator'], regex: /(\$)(\{)([#!]?)(\w+|[*@#?\-$!0_])(:[?+\-=]?|##?|%%?|,,?\/|\^\^?)?/, push: 'start' },
      ...
      s = function () {
        ...
        for (var t = 0; t < e.length; t++)
          if (e[t].token == 'variable.language') {
            e.splice(t, 0, { token: 'constant.language', regex: '(?:^(?:FROM|MAINTAINER|RUN|CMD|EXPOSE|ENV|ADD|ENTRYPOINT|VOLUME|USER|WORKDIR|ONBUILD|COPY|LABEL)\\b)', caseInsensitive: !0 });
            break;
          }
      };
